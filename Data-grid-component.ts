...
@Component({
    ...
    <table width="100%" height="100%" border="0" class="table table-striped table-hover"> 
        ...
        <tbody>
            <tr  *ngFor="let hero of herolist;index as idx;first as fst;last as lst;odd as od; even as ev">
                ...
                <td><img width="50" src={{hero.poster}} alt= {{hero.title}}/></td>
                <td>{{ hero.city }}</td>
                <td>{{ hero.ticketprice | currency :'INR':'symbol':'3.3-4' }}</td>
                <td>{{ hero.releasedate | date :'dd/MMM/yyyy' }}</td>
                <td> <button class="btn btn-primary btn-lg btn-block">{{ hero.movieslist.length }}</button></td>
            </tr>
        </tbody>
    </table>
    `
